...
1. **대상 서비스**: 과제는 어떤 서비스든 골라 그 서비스의 가이드 챗봇을 설계하는 것이었습니다. 저는 제가 실제로 진행 중인 메인 도메인 프로젝트 중 하나인 `storytowebtoon`을 선택했습니다 — 가상의 예시가 아니라 제가 직접 개발하고 있는 서비스에 도구 설계 원칙을 적용해보고 싶었습니다.
2. **챗봇의 성격**: 과제가 예시로 든 챗봇(반품/교환 정책 안내, 장비 대여 조회 등)은 대부분 사용자의 질문에 답만 해주는 **"안내형"** 챗봇입니다. 이번에 설계한 챗봇은 한 단계 더 나아가, 사용자의 요청을 받아 storytowebtoon의 실제 파이프라인(시놉시스 추출 → 시나리오화 → 4인 평가 → 종합 리포트)을 직접 수행해주는 **"오케스트레이션형"** 챗봇입니다. 정보를 안내만 하는 게 아니라, 도구를 순서대로 호출해 서비스의 핵심 작업 자체를 대신 처리해주는 방식이라 제 프로젝트에 더 잘 맞다고 판단했습니다.
...
이번 과제로 오늘 배운 내용을 정리해 **나만의 설계 체크리스트를 만들고, 이를 'DJ체크리스트'라 이름 붙여 앞으로 내 프로젝트에 적용해보기로 했다.**
